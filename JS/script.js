...
function renderPost(data, prepend = false) {
  ...
  if (data.file) {
    mediaHtml += `<div class="mb-3 media-btn-file" style="background-color: var(--secondary-color); color: var(--main-color); border-color: var(--border-color); border-radius: 16px; padding: 1.5rem; display: flex; align-items: center; gap: 12px; min-height:70px;">
      ...
      <a href="${
        data.file
      }" target="_blank" style="color: var(--b-color); word-break:break-all; text-decoration: underline; font-weight: 500;">${
    ...
    <div class="card-body mb-4 post-card" data-post-id="${data.id || ''}">
      <div class="d-flex align-items-center mb-3">
        <div class="profile-img-small me-3">
          ${
            data.avatar
              ? `<img src="${data.avatar}" alt="User image" class="rounded-circle" style="width:40px; height:40px;">`
              : '<i class="bi bi-person-fill"></i>'
          }
          
        </div>
        <div class="flex-grow-1">
          <h6 class="mb-0 post-author">${
...
function requireAuthAction(e, actionName = 'this action') {
  if (!currentUser || !currentUser.uid) {
    showNotification(
      `Please sign in to perform ${actionName}.`,
      'warning',
      2000
    )
    return false
...
if (postButton) {
  postButton.addEventListener('click', async (e) => {
    ...
    if (contentValue.length > 500) {
      showNotification(
        'Post content is too long. Maximum 500 characters.',
        'warning',
        2000
      )
      return
...
document.addEventListener('click', async (e) => {
  ...
  if (editBtn) {
    ...
    contentDiv.innerHTML = `
      <textarea class="form-control edit-post-textarea enhanced-textarea" style="width:100%; min-height:80px; margin-bottom:12px;">${oldContent.replace(
        /"/g,
        '&quot;'
      )}</textarea>
      <div>
  ...
  if (cancelEditBtn) {
    ...
    getDoc(doc(db, 'posts', postId)).then((postDoc) => {
      const contentDiv = postCard.querySelector('.post-content')
      if (contentDiv) {
        contentDiv.innerHTML = postDoc.exists()
...
function renderPopupPost(postData) {
  ...
  if (postData.image) {
    ...
    mediaHtml = `<div class="mb-2 file-attachment">
      ...
      <a href="${postData.file}" target="_blank">${
      postData.fileName || 'Download file'
    }</a>
    </div>`
  }
  ...
  postDiv.innerHTML = `
    <div class="d-flex align-items-center mb-2">
      ...
      <div>
        <h6 class="mb-0" style="color: var(--headline-color);">${
          postData.user ? postData.user : 'Unknown User'
        }</h6>
        <div class="text-muted small" style="color: var(--p-color);">${dateStr}</div>
      </div>
    </div>
    <div class="mb-2" style="color: var(--p-color);">${
...
async function renderPopupComments(postId) {
  ...
  commentsDiv.innerHTML =
    '<div class="loading-message">renderrenderrendering comments...</div>'
  try {
...
function renderNotificationPopup() {
  ...
  if (notificationList.length === 0) {
    notifListDiv.innerHTML =
      '<div class="no-notifications-message" style="text-align:center;color:var(--p-color,#94A1B2);padding:1.5rem 0;">No notifications.</div>'
    return
  }
...
notifBtn.addEventListener('click', () => {
  if (
    notifModalOverlay.style.display === 'none' ||
    notifModalOverlay.style.display === ''
  ) {
    openNotifModal()
  } else {
    closeNotifModal()
...
initializeUI()
loadAllPosts()
async function initializeCommentCount() {
  const querySnapshot = await getDocs(collection(db, 'posts'))
  for (const docSnap of querySnapshot.docs) {
...
initializeCommentCount()
  .then(() => {
    console.log('Comment counts initialized successfully.')
  })
  .catch((error) => {

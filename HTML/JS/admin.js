...
function getPlaceholderAvatar(width = 40, height = 40, name = "User") {
  return `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&size=${width}&background=7f5af0&color=ffffff&bold=true`
}
...
function renderUsersTable() {
  ...
  filteredUsers.forEach((user, index) => {
    ...
    try {
      const avatarUrl = user.avatar || getPlaceholderAvatar(40, 40, user.username || "User")
      ...
      row.innerHTML = `
        ...
        <td class="col-user">
          <div class="d-flex align-items-center">
            <img src="${avatarUrl}" 
                 ...
                 onerror="this.src='${getPlaceholderAvatar(40, 40, user.username || "User")}'">
            <div style="min-width: 0; flex: 1;">
              <div class="fw-bold" title="${user.username || "N/A"}">${user.username || "N/A"}</div>
        ...
        <td class="col-email" title="${user.email || "N/A"}">${user.email || "N/A"}</td>
        <td class="col-role">
...
function renderPostsTable() {
  ...
  filteredPosts.forEach((post) => {
    const authorAvatar = post.authorAvatar || getPlaceholderAvatar(35, 35, post.author || "Author")

    const row = document.createElement("tr")
    row.innerHTML = `
      <td class="col-id"><strong>${post.id}</strong></td>
      <td class="col-title">
        <div class="fw-bold mb-1" title="${post.title || "Không có tiêu đề"}">${highlightSearchTerm(post.title || "Không có tiêu đề", "postSearch")}</div>
      ...
      <td class="col-author">
        <div class="d-flex align-items-center">
          <img src="${authorAvatar}" 
               alt="Avatar" 
               class="avatar me-2" 
               onerror="this.src='${getPlaceholderAvatar(35, 35, post.author || "Author")}'">
          <span class="fw-semibold" title="${post.author || "N/A"}">${highlightSearchTerm(post.author || "N/A", "postSearch")}</span>
        </div>
      </td>
...
function addPost() {
  showNotification("Chức năng tạo bài viết sẽ được tích hợp với phần của Mai Anh", "info")
}
...
async function updateUserInfo() {
  ...
  try {
    const userData = {
      ...
      followers: Number.parseInt(document.getElementById("editFollowers")?.value) || 0,
      following: Number.parseInt(document.getElementById("editFollowing")?.value) || 0,
    }
...
async function createNewUser() {
  ...
  try {
    const userId = "admin_created_" + Date.now() + "_" + Math.random().toString(36).substr(2, 5)
    ...
    const userData = {
      ...
      role: role || "user",
      status: status || "active",
      followers: Number.parseInt(document.getElementById("newFollowers")?.value) || 0,
      following: Number.parseInt(document.getElementById("newFollowing")?.value) || 0,
    }
...
function exitAdminPage() {
  const confirmExit = confirm("Bạn có chắc chắn muốn thoát khỏi Admin Dashboard?")
